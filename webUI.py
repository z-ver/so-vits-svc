...
def vc_fn(sid, input_audio, vc_transform, auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,enhance,enhancer_adaptive_key):
    global model
    try:
        ...
        _audio = model.slice_inference(temp_path, sid, vc_transform, slice_db, cluster_ratio, auto_f0, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,enhance,enhancer_adaptive_key)
        model.clear_empty()
        os.remove(temp_path)
...
def vc_fn2(sid, input_audio, vc_transform, auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,text2tts,tts_rate,enhance,enhancer_adaptive_key):
    #使用edge-tts把文字转成音频
    ...
    a,b=vc_fn(sid, vc_input, vc_transform,auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,enhance,enhancer_adaptive_key)
...
with app:
    with gr.Tabs():
        with gr.TabItem("Sovits4.0"):
            ...
            enhance = gr.Checkbox(label="是否使用NSF_HIFIGAN增强,默认使用", value=True)
            ...
            def modelAnalysis(model_path,config_path,cluster_model_path,device):
                ...
                if debug:
                    ...
                    try:
                        ...
                        return "","异常信息:"+str(e)+"\n请排障后重试"
        vc_submit.click(vc_fn, [sid, vc_input3, vc_transform,auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,enhance,enhancer_adaptive_key], [vc_output1, vc_output2])
        vc_submit2.click(vc_fn2, [sid, vc_input3, vc_transform,auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,text2tts,tts_rate,enhance,enhancer_adaptive_key], [vc_output1, vc_output2])
